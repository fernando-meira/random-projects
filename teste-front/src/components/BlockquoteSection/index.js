import React from 'react';

import { Container } from './styles';

export default function BlockquoteSection() {
  return (
    <Container cite="http://www.worldwildlife.org/who/index.html">
      <span>"</span>For 50 years, WWF has been protecting the future of nature.
      The world's leading conservation organization, WWF works in 100 countries
      and is supported by 1.2 million members in the United States and close to
      5 million globally. <span>"</span>
    </Container>
  );
}
